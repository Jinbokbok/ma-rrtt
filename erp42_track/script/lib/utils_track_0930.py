...
#######Local Path#######
    def steering_angle(self, newpoints):
        ...
        if (len(newpoints.waypoints) <= 0) :
            self.steering = 0
            return self.steering, self.forward_point, self.avg_steering, self.steering_ori
        ...
        self.steering_ori= -atan(delta)*180/pi #deg

        if (self.steering_ori >= 10):
            self.steering = self.steering_ori +1
        elif (10>= self.steering_ori >= 8):
            self.steering = self.steering_ori 
        elif (self.steering_ori <= -10):
            self.steering = self.steering_ori -1
        elif (-10 <= self.steering_ori <= -8):
            self.steering = self.steering_ori  
        else:
